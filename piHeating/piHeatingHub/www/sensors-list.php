...
<head>
    ...
    <title>piHeatingHub - sensors</title>
    <style>
        .pbody {
...
<body class='pbody'>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (array_key_exists('new', $_POST)) {
            ...
            $subnet_scan = shell_exec("nmap -sP $subnet_cidr | grep report | grep -v router | rev | cut -d' ' -f 1 | rev | tr -d '()'");
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "<th class='tcol' width=1%><span class='tspan'>Id</span></th>";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $rrd_dir = '/home/pi/bin/piHeatingHub/data/s-';

            echo '<tr>';
            echo "<td class='dcolname'><span class='dspan'>$SENSOR_ID</span></td>";
            echo "<td class='dcolname'><span class='dspan'>$SENSOR_IP</span></td>";
            ...
            //echo $rrd_dir . $SENSOR_ID . ".rrd,\n" . $img_dir . $SENSOR_ID . $span . ".png,\n" . $span, $row["name"] . " last 24 hours,\n" . "80,\n" . "300";
            //echo "<br>\n" . $img_dir . $SENSOR_ID . $span . ".png";
            echo "</td>";
    ...
    <form method='post' action='sensors-list.php'>
        <input type='submit' name='new' value='Scan for new sensors' class='bgreen' /> &nbsp;&nbsp; <input type='button' onclick='location.href="/status.php";' value='Done' class='bgrey' />
    </form>
