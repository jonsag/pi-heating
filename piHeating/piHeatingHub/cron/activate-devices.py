...
# Physically turn ON/OFF GPIO pin for devices

cursorselect = cnx.cursor()
query = (
    "SELECT name, pin, active_level, value FROM devices WHERE active_level IS NOT NULL;"
)
cursorselect.execute(query)
results_devices = cursorselect.fetchall()
cursorselect.close()
...
for result in results_devices:
    ...
    print(
        (
            ...
            DEVICE_ACTIVE_LEVEL,
            DEVICE_VALUE,
            1 - (DEVICE_ACTIVE_LEVEL ^ DEVICE_VALUE),
        )
    )
