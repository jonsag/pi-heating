...
def active_devices(cursor, cnx, verbose):
    ...
    for result in results_devices:
        ...
        if verbose:
            print(
                "Device %s %s, Pin %s, Active %s, Value %s, Pin %s"
                % (
                    DEVICE_ID,
                    DEVICE_NAME,
                    DEVICE_PIN,
...
def process_schedules(cursor, cnx, now, process, verbose):
    ...
    for result in results_schedules:
        ...
        if verbose:
            print(
                "Start time: %s, End time: %s, Test time: %s"
                % (
                    SCHED_START_STR,
                    SCHED_END_STR,
                    SCHED_TEST_TIME,
        ...
        cursor = db_create_cursor(cnx)
        query = (
            "SELECT * FROM sensors INNER JOIN sched_sensor ON sensors.id=sched_sensor.sensor_id AND sched_sensor.sched_id='"
            + SCHED_ID
            + "'"
        )
        ...
        cursor = db_create_cursor(cnx)
        query = (
            "SELECT * FROM modes INNER JOIN sched_mode ON modes.id=sched_mode.mode_id AND sched_mode.sched_id='"
            + SCHED_ID
            + "'"
        )
        ...
        cursor = db_create_cursor(cnx)
        query = (
            "SELECT timers.value, sched_timer.value FROM timers INNER JOIN sched_timer ON timers.id = sched_timer.timer_id AND sched_timer.sched_id ='"
            + SCHED_ID
            + "'"
        )
        ...
        for result in results_timers:
            ...
            if TEST_VALUE and TIMER_VALUE > 0:
                ...
                not TEST_VALUE and TIMER_VALUE == 0
            ):  # # Thanks to Alan Riley for spotting this.
                if verbose:
    ...
    for conclusion in conclusions:
        ...
        if (
            conclusion["testToday"]
            and conclusion["modeCalls"]
            and conclusion["timerCalls"]
        ):
